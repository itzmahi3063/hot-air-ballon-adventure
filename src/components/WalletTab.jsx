...
export default function WalletTab({ user, onWithdrawSubmit }) {
  ...
  const handleWithdraw = async (e) => {
    ...
    try {
      ...
      setStatusMessage({ type: "error", text: err.message || "Withdrawal failed, try again" });
    } finally {
        ...
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
          <div className="w-full max-w-sm card-3d rounded-3xl p-5 shadow-gold-glow flex flex-col gap-4 relative">
            ...
            <button
              ...
              className="w-full py-3.5 rounded-full btn-3d-gold text-xs font-black uppercase tracking-wider mt-1 cursor-pointer disabled:opacity-60"
            >
