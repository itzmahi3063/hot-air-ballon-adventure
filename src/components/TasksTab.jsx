...
export default function TasksTab({ onRewardEarned }) {
  const [activeCategory, setActiveCategory] = useState("daily");
  const [watchAds, setWatchAds] = useState(initialWatchAds);
  const [telegramTasks, setTelegramTasks] = useState(initialTelegramTasks);
  ...
  const handleWatchAd = (ad) => {
    ...
    setTimeout(() => {
      setWatchAds((prev) =>
        prev.map((item) => {
          if (item.id === ad.id) {
            const nextCompleted = Math.min(item.totalSlots, item.completedSlots + 1);
            return { ...item, completedSlots: nextCompleted };
          }
          return item;
        })
      );
      ...
      onRewardEarned({
        ...
        label: `Watched ${ad.title} Ad (+${ad.rewardCoins} points)`,
      });
  ...
  const handleVerifyTelegram = (task) => {
    ...
    setTimeout(() => {
      ...
      onRewardEarned({
        coins: task.reward,
        label: `Verified ${task.title} (+${task.reward.toFixed(2)})`,
      });
